...
from app.devices.device_manager import DeviceManager, DeviceState
...
class DeviceManagePanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        self._dev_tree.setFocusPolicy(Qt.FocusPolicy.NoFocus)
        self._dev_tree.setEditTriggers(
        ...
        # 行高 28px: 行内 QSpinBox 上下按钮热区随之增大 (行高24时仅 11/12px, 难以点中)
        self._ch_table.verticalHeader().setDefaultSectionSize(28)
        self._ch_table.setSelectionBehavior(
            QAbstractItemView.SelectionBehavior.SelectRows)
        self._ch_table.setFocusPolicy(Qt.FocusPolicy.NoFocus)
        self._ch_table.setCurrentCell(-1, -1)
        self._ch_table.setEditTriggers(
            QAbstractItemView.EditTrigger.NoEditTriggers)
        self._ch_table.setItemDelegate(_NoEditDelegate(self._ch_table))
    ...
    def _create_device(self, vendor, bus_type):
        ...
        if self._dm.active_project is None:
            QMessageBox.warning(self, tr("提示"), tr("请先创建或选择一个工程"))
            return
        from app.devices.device_dialog import DeviceSelectDialog
    ...
    def _on_channel_param(self, key: str):
        ...
        if self._dm.channel_state(key) == "connected":
            ...
            return
        from app.devices.device_dialog import ChannelParamDialog
