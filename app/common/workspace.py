...
- 保存目录由 AppConfig.save_dir 配置; 为空时默认 = 运行/安装路径/data
  (即 main.py / app 包所在工程根目录下的 data 目录)。
"""
...
# 运行/安装路径: app/common/workspace.py -> 上三级为工程根 (busforge/)
APP_RUN_DIR = Path(__file__).resolve().parents[2]
# 运行数据目录: 会话索引/工程文件/记录输出默认集中于此
DATA_DIR = APP_RUN_DIR / "data"
