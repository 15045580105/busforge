...
def read_log_file(file_path: str, channel_key: str = "") -> list[FrameDTO]:
    """读取 ASC/BLF 日志并转换为 Trace 使用的 FrameDTO 列表."""
    ...
    if suffix == ".blf":
        ...
        raise ValueError(f"不支持的日志格式: {suffix or '无扩展名'}")

    frames = []
    for message in reader:
        data = bytes(message.data)
        frames.append(FrameDTO(
            channel_key=channel_key or f"CAN{message.channel or 1}",
