...
from PySide6.QtCore import QTimer, Qt, QEvent, QPoint, QSize, Signal
...
class LoggerPanel(QWidget):
    ...
    send_to_trace_requested = Signal(str)
    ...
    def __init__(self, channel_key: str = "", parent=None):
        ...
        self._file_tree.setContextMenuPolicy(
    ...
    def _on_file_context_menu(self, pos):
        ...
        if not path:
            return
        menu = QMenu(self)
        send_action = menu.addAction(tr("发送到 Trace"))
        send_action.triggered.connect(
            lambda: self.send_to_trace_requested.emit(path))
        menu.addSeparator()
        menu.addAction(tr("导出")).triggered.connect(self._export_files)
        menu.addAction(tr("删除")).triggered.connect(self._delete_files)
