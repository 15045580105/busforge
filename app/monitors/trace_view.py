...
from PySide6.QtWidgets import (
    ...
    QListWidget, QListWidgetItem, QFrame, QComboBox, QSpinBox, QFileDialog,
...
from app.datahub.data_hub import (
...
class _LogReaderThread(QThread):
    ...
    def __init__(self, path: str, channel_key: str = "", parent=None):
    ...
    def run(self):
        try:
            from app.recorder.can_logger import read_log_file
            self.loaded.emit(read_log_file(self._path, self._channel_key), self._path)
...
class TraceView(QWidget):
    ...
    def __init__(self, channel_key: str = "", parent=None):
        ...
        self._file_frames: list[FrameDTO] = []
        self._file_filtered: list[FrameDTO] = []
        self._file_page = 1
        self._file_page_size = _PAGE_SIZE
        self._file_source = ""
    ...
    def _setup_ui(self):
        ...
        self._import_btn = _ToolButton("import", "导入日志", "导入日志")
        self._import_btn.clicked.connect(self._choose_log_file)
        ...
        self._page_prev_btn = QPushButton(tr("上一页"))
        self._page_prev_btn.clicked.connect(lambda: self._show_file_page(self._file_page - 1))
        page_bar.addWidget(self._page_prev_btn)
        self._page_edit = QLineEdit("1")
        self._page_edit.setFixedWidth(60)
        self._page_edit.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self._page_edit.returnPressed.connect(self._goto_file_page)
        page_bar.addWidget(self._page_edit)
        self._page_label = QLabel("/ 0")
        page_bar.addWidget(self._page_label)
        self._page_next_btn = QPushButton(tr("下一页"))
        self._page_next_btn.clicked.connect(lambda: self._show_file_page(self._file_page + 1))
        page_bar.addWidget(self._page_next_btn)
        page_bar.addWidget(QLabel(tr("每页")))
        ...
        page_bar.addWidget(self._page_info, 1)
    ...
    def import_file(self, path: str):
        if self._file_thread is not None and self._file_thread.isRunning():
            return
        self._page_info.setText(tr("正在读取: ") + path)
        self._file_thread = _LogReaderThread(path, self._channel_key, self)
        self._file_thread.loaded.connect(self._on_file_loaded)
        self._file_thread.failed.connect(self._on_file_failed)
        self._file_thread.finished.connect(self._file_thread.deleteLater)
        self._file_thread.start()

    def _on_file_loaded(self, frames, path: str):
        self._file_frames = list(frames)
        self._file_source = path
        self._file_page = 1
        self._rebuild_file_page()

    def _on_file_failed(self, message: str):
        self._page_info.setText(tr("导入失败: ") + message)

    def _rebuild_file_page(self):
        self._file_filtered = [f for f in self._file_frames
                               if self._matches_filter(f)]
        total_pages = max(1, (len(self._file_filtered) + self._file_page_size - 1)
                          // self._file_page_size)
        self._file_page = max(1, min(self._file_page, total_pages))
        self._show_file_page(self._file_page)

    def _show_file_page(self, page: int):
        if not self._file_frames:
            return
        total_pages = max(1, (len(self._file_filtered) + self._file_page_size - 1)
                          // self._file_page_size)
        self._file_page = max(1, min(page, total_pages))
        start = (self._file_page - 1) * self._file_page_size
        batch = self._file_filtered[start:start + self._file_page_size]
        self._table.setUpdatesEnabled(False)
        try:
            self._table.clear()
            self._ow_items.clear()
            self._last_ts.clear()
            self._t0 = batch[0].timestamp if batch else None
            for frame in batch:
                self._append_scroll(frame, cap=False)
        finally:
            self._table.setUpdatesEnabled(True)
        self._page_edit.setText(str(self._file_page))
        self._page_label.setText(f"/ {total_pages}")
        self._page_prev_btn.setEnabled(self._file_page > 1)
        self._page_next_btn.setEnabled(self._file_page < total_pages)
        self._page_info.setText(
            f"{len(self._file_filtered)} / {len(self._file_frames)}")

    def _goto_file_page(self):
        try:
            page = int(self._page_edit.text())
        except ValueError:
            page = self._file_page
        self._show_file_page(page)

    def _page_size_changed(self, _index):
        self._file_page_size = self._page_size_combo.currentData()
        self._rebuild_file_page()

    def dragEnterEvent(self, event):
        if any(url.toLocalFile().lower().endswith((".asc", ".blf"))
    ...
    def dropEvent(self, event):
        for url in event.mimeData().urls():
            ...
            if path.lower().endswith((".asc", ".blf")):
                self.import_file(path)
                break
        event.acceptProposedAction()
    ...
    def closeEvent(self, event):
        if self._file_thread is not None and self._file_thread.isRunning():
            self._file_thread.requestInterruption()
            self._file_thread.wait(1000)
        self._hub.unregister(self._sub_id)
